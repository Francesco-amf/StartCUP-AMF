...
export default function EvaluatorPenaltyAssigner() {
  ...
  const handleAssignPenalty = async () => {
    ...
    try {
      const response = await fetch('/api/evaluator/assign-penalty', {
        method: 'POST',
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="bg-gradient-to-br from-[#0A1E47]/60 to-[#001A4D]/60 backdrop-blur-sm border-2 border-[#00E5FF]/40 rounded-lg p-6 space-y-4">
          ...
          {selectedPenaltyType && (
            <div>
              ...
              <input
                ...
                className="w-full h-2 bg-[#0A1E47]/50 rounded-lg appearance-none cursor-pointer accent-[#FF3D00]"
              />
          ...
          {/* Motivo (Obrigatório) */}
          <div>
            <label className="block text-[#00E5FF] font-semibold mb-2">Descrição da Infração *</label>
            <textarea
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="Descreva detalhadamente o motivo da penalidade..."
              rows={3}
          ...
          <Button
            ...
            disabled={!selectedTeam || !selectedPenalty || pointsDeduction === 0 || !reason || loading}
